function [Out_time] = function2(ind1,Res2,I,d0,nt,nk,f2,SampleRate,n,n1,Plength,Xcm,a0Out,Rc,Lc,mMassi,opts,g_acel,movTFz,movtFz,Fg_m,Dampf,Dampc_m,Dampf1,Amp,ind2,Aper,ind3,f4)
...
t_1=zeros(nt+1,nk);y_1=zeros(nt+1,3,nk); %Pow=zeros(nt+1,nk); AvPow=zeros(1,nk); mFzi=zeros(nt+1,nk); iFzi=mFzi; Ffic_m=Pow; FficVel_m=Pow; AFfic_m=AvPow; AvFficVel_m=AvPow; 
...
t_1(:,k)=t1;            % Time
y_1(:,1:3,k)=y;           % Output [delta, ddelta/dt, I] vs time
% Pow(:,k)=y_1(:,3,k).*y_1(:,4,k); % Output power vs time (I*V)
% AvPow(k)=(sum(Pow(:,k))-(1/2)*(Pow(1,k)+Pow(end,k)))/nt; % Output average power 

Out_time=[t_1(:,k),y_1(:,:,k)]; % [time (s), position (m), velocity (m/s), current (A)]
filename = ['f_',num2str(ff2),'_X_',num2str(Amp(ind2)*1E-3),'_R_',num2str(Res),'_A_',num2str(Aper(ind3)),'_',num2str(f4(k)),'.mat'];
save(filename,'Out_time')
...
% Ou1(ind1,ind2)=Y1_1max(k);
% Ou2(ind1,ind2)=Yf1_1(k);
% 
%     end
%     ind1
% end
...
% % AvPow_AvPowIn=100*AvPow./(mMassi.*AvFficVel_m);  % Average output power / Average free magnet (%)
